...
namespace Omicron.Model
{
    public class Testerwith4item
    {
        ...
        public double[] TestIdle { set; get; } = new double[4] { 0, 0, 0, 0 };
        public double[] TestCycle { set; get; } = new double[4] { 0, 0, 0, 0 };
        ...
        Stopwatch idlesw1, idlesw2;
        bool idleswflag1, idleswflag2;
        public bool TimeoutStauts = false;
        ...
        private void UpdateTester2(int rst, int index_i)
        {
            /*result = 0 -> Ng
* result = 1 -> Pass
* result = 2 -> Timeout
*/
        ...
        public async void Start1(StartProcessedDelegate callback)
        {
            ...
            Func<Task> startTask = () =>
            {
                return Task.Run(async () =>
                {
                    ...
                    idleswflag1 = false;
                    idlesw1.Stop();
                    Inifile.INIWriteValue(iniTesterResutPath, "Tester" + (Index * 2 + 0).ToString(), "TestIdle", TestIdle[0].ToString());
                    TestCycle[0] = TestSpan[0] + TestIdle[0];
                    Inifile.INIWriteValue(iniTesterResutPath, "Tester" + (Index * 2 + 0).ToString(), "TestCycle", TestCycle[0].ToString());
                    ...
                    if (errorcode[0].Contains("5065") || errorcode[0].Contains("5073"))
                    {
                        needRecord = false;
                    }
                    else
        ...
        private void RunLoop()
        {
            ...
            while (true)
            {
                if (idleswflag1)
                {
                    TestIdle[0] = Math.Round(idlesw1.Elapsed.TotalSeconds, 2);
                    if (TestIdle[0] > 99.99)
                    {
                        idleswflag1 = false;
                        idlesw1.Stop();
                ...
                if (idleswflag2)
                {
                    TestIdle[1] = Math.Round(idlesw2.Elapsed.TotalSeconds, 2);
                    if (TestIdle[1] > 99.99)
                    {
                        idleswflag2 = false;
                        idlesw2.Stop();
